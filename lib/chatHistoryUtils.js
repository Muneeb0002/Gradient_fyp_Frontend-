...
export function formatSidebarDate(iso) {
  ...
  const startOfChat = new Date(d.getFullYear(), d.getMonth(), d.getDate());
  const dayDiff = Math.floor((startOfToday - startOfChat) / 86400000);
...
export function chatToSidebarItem(chat) {
  return {
    id: chat._id,
    title: truncate(chat.query, 44),
    preview: answerPreview(chat.answer, 72),
    time: formatSidebarDate(chat.createdAt),
    marks: chat.marks,
    mode: chat.mode,
  };
}
...
/** Split answer into plain paragraphs for chat bubbles */
export function splitAnswerParagraphs(answer) {
  return sanitizeDisplayText(answer || "")
    .split(/\n+/)
